 ...
 *
 * Uses bun:sqlite directly (no sqlite3 CLI), platform-aware service checks.
 */
...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.jimmyclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.jimmyclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active jimmyclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('jimmyclaw')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(envFile)) {
    ...
    if (/^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|Z_AI_API_KEY)=/m.test(envContent)) {
  ...
  // 5. Check registered groups (using bun:sqlite, not sqlite3 CLI)
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'jimmyclaw', 'mount-allowlist.json'))) {
