...
function setupLaunchd(projectRoot: string, nodePath: string, homeDir: string): void {
  const plistPath = path.join(homeDir, 'Library', 'LaunchAgents', 'com.jimmyclaw.plist');
...
<dict>
  ...
  try {
    ...
    serviceLoaded = output.includes('com.jimmyclaw');
  } catch {
...
/**
 * Kill any orphaned jimmyclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned jimmyclaw processes');
  } catch {
...
function setupSystemd(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/jimmyclaw.service';
    ...
    unitPath = path.join(unitDir, 'jimmyclaw.service');
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned jimmyclaw processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable jimmyclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start jimmyclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active jimmyclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  const wrapperPath = path.join(projectRoot, 'start-jimmyclaw.sh');
  const pidFile = path.join(projectRoot, 'jimmyclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-jimmyclaw.sh — Start JimmyClaw without systemd',
    ...
    '    echo "Stopping existing JimmyClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting JimmyClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/jimmyclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/jimmyclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "JimmyClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/jimmyclaw.log"`,
  ];
