...
│  │    • mcp__jimmyclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── jimmyclaw.log               # Host stdout
│   └── jimmyclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.jimmyclaw.plist         # macOS service configuration
```
...
- `{{PROJECT_ROOT}}` - Absolute path to your jimmyclaw installation
...
JimmyClaw uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: jimmyclaw (scheduler)
   │
   ▼
...
JimmyClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__jimmyclaw__schedule_task]
        {
...
### JimmyClaw MCP (built-in)

The `jimmyclaw` MCP server is created dynamically per agent call with the current group's context.
...
JimmyClaw runs as a single macOS launchd service.

### Startup Sequence

When JimmyClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned JimmyClaw containers from previous runs
...
### Service: com.jimmyclaw

**launchd/com.jimmyclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/jimmyclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/jimmyclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.jimmyclaw.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep jimmyclaw

# View logs
tail -f logs/jimmyclaw.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep jimmyclaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; JimmyClaw auto-starts container runtime but may fail |
...
- `logs/jimmyclaw.log` - stdout
- `logs/jimmyclaw.error.log` - stderr
