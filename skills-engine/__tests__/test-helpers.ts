...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'jimmyclaw-test-'));
}

export function setupNanoclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.jimmyclaw', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.jimmyclaw', 'backup'), { recursive: true });
}
