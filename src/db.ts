...
export function getNewMessages(
  ...
  const rows = stmt.all(lastTimestamp, ...jids, `${botPrefix}:%`) as NewMessage[];
...
export function getMessagesSince(
  ...
  const sqlStr = `
    ...
    ORDER BY timestamp
  `;
  return rawDb.prepare(sqlStr).all(chatJid, sinceTimestamp, `${botPrefix}:%`) as NewMessage[];
}
...
export function getTasksForGroup(groupFolder: string): ScheduledTask[] {
  return db
    .select()
    .from(schema.scheduledTasks)
    .where(eq(schema.scheduledTasks.group_folder, groupFolder))
    .orderBy(desc(schema.scheduledTasks.created_at))
...
export function deleteTask(id: string): void {
  db.delete(schema.taskRunLogs).where(eq(schema.taskRunLogs.task_id, id)).run();
  ...
  return db
    ...
    .where(
      and(
        ...
        isNotNull(schema.scheduledTasks.next_run),
        lte(schema.scheduledTasks.next_run, now),
      ),
    )
    .orderBy(schema.scheduledTasks.next_run)
    .all() as ScheduledTask[];
}
...
export function getRegisteredGroup(jid: string): (RegisteredGroup & { jid: string }) | undefined {
  const row = db
    .select()
    .from(schema.registeredGroups)
    .where(eq(schema.registeredGroups.jid, jid))
    .get();
...
export function getAllRegisteredGroups(): Record<string, RegisteredGroup> {
  const rows = db.select().from(schema.registeredGroups).all();
...
function migrateJsonState(): void {
  ...
  const routerStateData = migrateFile('router_state.json') as {
    last_timestamp?: string;
    last_agent_timestamp?: Record<string, string>;
  } | null;
  if (routerStateData) {
    if (routerStateData.last_timestamp) {
      setRouterState('last_timestamp', routerStateData.last_timestamp);
    }
    if (routerStateData.last_agent_timestamp) {
      setRouterState('last_agent_timestamp', JSON.stringify(routerStateData.last_agent_timestamp));
    }
  }

  const sessionsData = migrateFile('sessions.json') as Record<string, string> | null;
  if (sessionsData) {
    for (const [folder, sessionId] of Object.entries(sessionsData)) {
