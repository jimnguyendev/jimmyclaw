...
// Helper: advance fake timers and flush the microtask queue so async
// callbacks triggered by setTimeout have a chance to run.
async function advanceTimers(ms: number): Promise<void> {
  jest.advanceTimersByTime(ms);
  // Yield to the event loop multiple times to let chained promises settle
...
describe('GroupQueue', () => {
  ...
  it('only runs one container per group at a time', async () => {
    ...
    const processMessages = jest.fn(async (_groupJid: string) => {
      concurrentCount++;
  ...
  it('stops retrying after MAX_RETRIES and resets', async () => {
    ...
    for (let i = 0; i < retryDelays.length; i++) {
      await advanceTimers(retryDelays[i] + 10);
      expect(callCount).toBe(i + 2);
    }

    // After 5 retries (6 total calls), should stop — no more retries
    const countAfterMaxRetries = callCount;
    await advanceTimers(200000); // Wait a long time
  ...
  it('does NOT preempt active container when not idle', async () => {
    ...
    const writeFileSync = (fs.default as any).writeFileSync as jest.Mock;
    const closeWrites = writeFileSync.mock.calls.filter(
      (call: any) => typeof call[0] === 'string' && call[0].endsWith('_close'),
    );
