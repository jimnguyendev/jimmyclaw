...
// Mock db
mock.module('../db.js', () => ({
  getLastGroupSync: jest.fn(() => null),
  setLastGroupSync: jest.fn(),
  updateChatName: jest.fn(),
}));

// Mock transcription
mock.module('../transcription.js', () => ({
  isVoiceMessage: jest.fn((msg: any) => msg.message?.audioMessage?.ptt === true),
  transcribeAudioMessage: jest.fn().mockResolvedValue('Hello this is a voice message'),
}));
...
mock.module('@whiskeysockets/baileys', () => {
  return {
    ...
    Browsers: { macOS: jest.fn(() => ['macOS', 'Chrome', '']) },
    DisconnectReason: {
      loggedOut: 401,
      badSession: 500,
...
function createTestOpts(overrides?: Partial<WhatsAppChannelOpts>): WhatsAppChannelOpts {
  return {
    onMessage: jest.fn(),
    onChatMetadata: jest.fn(),
    registeredGroups: jest.fn(() => ({
      'registered@g.us': {
        name: 'Test Group',
        folder: 'test-group',
        trigger: '@Andy',
...
describe('WhatsAppChannel', () => {
  beforeEach(() => {
    ...
    (getLastGroupSync as ReturnType<typeof jest.fn>).mockReturnValue(null);
  });
  ...
  describe('message handling', () => {
    ...
    it('falls back when transcription returns null', async () => {
      (transcribeAudioMessage as ReturnType<typeof jest.fn>).mockResolvedValueOnce(null);
    ...
    it('falls back when transcription throws', async () => {
      (transcribeAudioMessage as ReturnType<typeof jest.fn>).mockRejectedValueOnce(new Error('API error'));
  ...
  describe('group metadata sync', () => {
    ...
    it('skips sync when synced recently', async () => {
      ...
      (getLastGroupSync as ReturnType<typeof jest.fn>).mockReturnValue(
    ...
    it('skips groups with no subject', async () => {
      ...
      (updateChatName as ReturnType<typeof jest.fn>).mockClear();
